/**
  ...
  * @Class Comlink
  *
  * Comlink is a client wrapper for SWGOH Comlink API to make connecting and requesting information from it easier. This version provides support for both Comlink for Github and the hybrid method of using a hosted Comlink and the swgoh-utils/gamedata
  *
  * Available Methods
  * - .fetchPlayers()
  * - .fetchData()
 ...
 * - NOTE: For github you can only use the guild id.
 * @param {Bool} enums - Optional: Flag to return enum values in the response
...
Comlink.prototype.fetchGuilds = function(id, enums = false, isPlayerID = false){
  ...
  if(this.usingGithub){
    id.forEach(guild => {
      request.push(this.requestParameters_(this.github_url + "/guild/" + guild + "_PROFILE.json","{}","GET"));
    });
    response = this.fetchAllAPI_(request);
...
Comlink.prototype.fetchData = function(collections = []){
  ...
  for(let r=0; r < response.length; r++){
    if(this.usingGithub){
      requestedData[dataOrder[r]] = response[r];
    } else{
...
*/
Comlink.prototype.fetchEvents = function(){
  if(this.usingGithub){
    return this.fetchAPI_(this.url_data + "currentEvents.json","{}", "GET");
  }else{
...
Comlink.prototype.fetchGuildByName = function(name,count = 10000, enums = false){
  if(this.usingGithub){ throw new Error("Comlink for Github does not support using the getGuilds endpoint. If you have a use in mind that is not already implemented for it please leave a suggestion for the developer on Discord."); }
...
Comlink.prototype.fetchGrandArenaBracket = function(eventID, groupID, enums = false){
  if(this.usingGithub){ throw new Error("Comlink for Github does not support using the getLeaderboard endpoint. If you have a use in mind that is not already implemented for it please leave a suggestion for the developer on Discord."); }
...
Comlink.prototype.fetchGuildLeaderboards = function(type, month = 0, count = 200, event = null, enums = false){
  if(this.usingGithub){ throw new Error("Comlink for Github does not support using the getGuildLeaderboard endpoint. If you have a use in mind that is not already implemented for it please leave a suggestion for the developer on Discord."); }
...
Comlink.prototype.fetchEnums = function(){
  if(this.usingGithub){ throw new Error("Comlink for Github does not support using the enums endpoint. If you want data returned with enums you must do it when running the available functions offered by Comlink for Github."); }
...
Comlink.prototype.fetchLocalization = function(language = this.language){
  ...
  var localization = [];
  if(this.usingGithub){
    localization = this.fetchAPI_(this.github_url + "/localization/" + language, "{}", "GET");
  } else{
...
Comlink.prototype.getLangFileName_ = function(language){
  ...
  var langOptions = {
    "CHS_CN": (useGithub) ? "CHS_CN.json" : "Loc_CHS_CN.txt.json",
    "CHT_CN": (useGithub) ? "CHT_CN.json" : "Loc_CHT_CN.txt.json",
    "ENG_US": (useGithub) ? "ENG_US.json" : "Loc_ENG_US.txt.json",
    "FRE_FR": (useGithub) ? "FRE_FR.json" : "Loc_FRE_FR.txt.json",
    "GER_DE": (useGithub) ? "GER_DE.json" : "Loc_GER_DE.txt.json",
    "IND_ID": (useGithub) ? "IND_ID.json" : "Loc_IND_ID.txt.json",
    "ITA_IT": (useGithub) ? "ITA_IT.json" : "Loc_ITA_IT.txt.json",
    "JPN_JP": (useGithub) ? "JPN_JP.json" : "Loc_JPN_JP.txt.json",
    "KOR_KR": (useGithub) ? "KOR_KR.json" : "Loc_KOR_KR.txt.json",
    "POR_BR": (useGithub) ? "POR_BR.json" : "Loc_POR_BR.txt.json",
    "RUS_RU": (useGithub) ? "RUS_RU.json" : "Loc_RUS_RU.txt.json",
    "SPA_XM": (useGithub) ? "SPA_XM.json" : "Loc_SPA_XM.txt.json",
    "THA_TH": (useGithub) ? "THA_TH.json" : "Loc_THA_TH.txt.json",
    "TUR_TR": (useGithub) ? "TUR_TR.json" : "Loc_TUR_TR.txt.json"
  }
